...
- `kakao-map` 스킬(k-skill-proxy가 중계하는 카카오 공식 Local REST API)로 확인한 장소 주소를 기준으로 주변 가맹점 조회. 장소 해석에 카카오맵 웹 스크래핑을 쓰지 않는다.
- 가맹점별 카카오맵 표시 평점을 번들 헬퍼(`scripts/kakao-place-rating.mjs`)로 조회해 표에 함께 표시. 확인할 수 없는 평점은 추측하지 않고 `미확인`으로 표시
- eat.pl 잇플 API가 반환하는 최대 100건의 매출 상위 순 결과 확인
- 장르, 동네 맛집, 외국인 방문, 회식 적합성, 영업 예상 시간과 지도 정보 대조
- 모든 행의 가맹점명 바로 오른쪽에 `web.paybooc.ai` eat.pl 상세 링크 첨부
...
표에는 가맹점명, eat.pl 상세 링크(가맹점명 오른쪽), 주소, 업종, 전국·지역 매출 상위 %, 카카오맵 평점(지역 매출 상위 오른쪽, 확인 불가 시 `미확인`), 동네 맛집 여부, 외국인·회식 적합 여부, 영업 예상 시간, 혼잡 시간, 주요 고객층을 표시한다. 상세 링크가 없는 행은 출력하지 않는다. 주소 텍스트 검색이므로 실제 반경은 지도 서비스로 교차 확인한다.
